...
export const JURISDICTIONS: readonly JurisdictionRecord[] = [
  ...
  },
  {
    ...
    notes:
      'Nigeria is a FEDERATION (36 states + the Federal Capital Territory) under the 1999 Constitution, and — counter-intuitively for a federation — its police and prisons are CENTRALISED at the FEDERAL level, while courts, prosecution and criminal law operate at BOTH levels. Legislative power is split by the Second Schedule: an Exclusive Legislative List (federal only, s 4(2)-(3)), a Concurrent List (both), and residual matters (states, s 4(7)). Mapping the four functions: POLICING is federal — Exclusive List item 45 and s 214(1), "no other police force shall be established for the Federation or any part thereof" (so policingScope `own` here, `none` at state level; no state runs a police force); CORRECTIONS is federal — Exclusive List item 48 "Prisons" (correctionalScope `own` here, `none` at state level; the Nigerian Correctional Service is federal); COURTS operate at both levels — s 6(5) makes the federal Supreme Court, Court of Appeal, Federal High Court and FCT High Court AND the State High Courts, Sharia Courts of Appeal and Customary Courts of Appeal the superior courts of record, so courtScope is `shared` (unlike India\'s fully integrated single hierarchy — Nigeria\'s states genuinely run their own superior courts, though appointment, discipline and funding are unified under the federal National Judicial Council, s 153 + Third Schedule, and the apex is federal); PROSECUTION is dual — the Attorney-General of the Federation for federal offences (s 174) and each State Attorney-General for state offences (s 211), so `shared`; CRIMINAL LAW is largely residual/state — it is not an Exclusive-List item, so states legislate their own substantive codes (the Criminal Code in the south, the Penal Code in the north, and Sharia Penal Codes in twelve northern states), while the Federation legislates federal offences, so legalSystemScope is `shared`. Two illustrative state records are modelled — Kano (a northern Sharia-penal-code state) and Lagos (a southern common-law state) — to show that the sub-national tier runs courts, prosecution and criminal law but NOT police or prisons, the INVERSE of the India/Mexico/Argentina federations where the states also run their own police and prisons. Facts kept in prose: constitutional review is DIFFUSE (any superior court of record; no separate constitutional court); the Attorney-General is a Cabinet Minister (s 150), so prosecution is not structurally independent of the executive; and a Constitution (Alteration) (State Police) Bill was, as of late July 2026, passed by both chambers but NOT yet law (it awaits ratification by at least 24 State Houses of Assembly and presidential assent) — if enacted it would end the s 214(1) single-force monopoly and require per-state police records.',
    status: 'published',
  },
  {
    id: 'ng-kn',
    ...
    notes:
      "A representative northern state, modelled to show the sub-national side of Nigeria's split: Kano runs its own State High Court, Sharia Court of Appeal and Customary Court of Appeal (courtScope `own`), its own State Attorney-General prosecutes state offences under s 211 (prosecutionScope `own`), and it legislates its own substantive criminal law — Kano is one of the twelve northern states that adopted a Sharia Penal Code from 1999–2001 (per Human Rights Watch, by attribution), so legalSystemScope is `own`. But policingScope and correctionalScope are `none`: there is no Kano state police (the Nigeria Police Force is the sole force, s 214(1)) and no Kano state prison service (prisons are Exclusive List item 48, run by the federal Nigerian Correctional Service). Kano's specific institutional detail has NOT been researched; it is modelled to make the criminal-law variant and the federal-police/federal-prisons split concrete, not to assert anything particular to the state.",
    status: 'published',
  },
  {
    id: 'ng-la',
    ...
    notes:
      "A representative southern state, modelled for the same reason as Kano and to give the common-law side of the contrast: Lagos runs its own State High Court and Customary Court of Appeal (courtScope `own`), its own State Attorney-General prosecutes state offences under s 211 (prosecutionScope `own`), and its substantive criminal law is the common-law Criminal Code tradition of the southern states rather than a Sharia Penal Code (legalSystemScope `own`). As with every state, policingScope and correctionalScope are `none` — the police (s 214(1)) and prisons (Exclusive List item 48) are federal. Lagos's specific institutional detail has NOT been researched beyond this structural placement.",
    status: 'published',
  },
];
