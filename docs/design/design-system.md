...
| `SiteNav`           | Primary navigation: desktop links plus the disclosure panel below 896 px | **The only client component.** Needs `usePathname` for `aria-current` and disclosure state for the panel. `aria-expanded`, `aria-controls`, focus moved into the panel on open and restored to the toggle on close, `Escape` closes. The toggle keeps the accessible name "Menu" in both states — a disclosure control conveys state through `aria-expanded`, not by renaming itself |
